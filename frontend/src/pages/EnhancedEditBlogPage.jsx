  ...
  const loadBlog = async () => {
    try {
      ...
      setFormData({
        ...
        category: blog.category || 'Manga',
        genres: blog.genres || [],
        status: blog.status || 'Ongoing',
        adminRating: blog.adminRating || 0,
        episodes: blog.episodes || 0,
        chapters: blog.chapters || 0,
        alternativeNames: blog.alternativeNames?.join(', ') || '',
        readingReview: blog.readingReview || '',
        releaseDate: blog.releaseDate ? new Date(blog.releaseDate).toISOString().split('T')[0] : ''
      });
    } catch (error) {
...
export default EnhancedEditBlogPage;